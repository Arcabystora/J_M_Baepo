...
class ManagerCheck(APIView):
    def post(self, request):
        ...
        # 무한 루프를 백그라운드 스레드로 실행합니다.
        loop_thread = threading.Thread(target=self.check_status, args=(client_name, push_point, current_point))
        loop_thread.start()

        # 메인 스레드에서는 일정 시간 후에 백그라운드 스레드를 중단시킵니다.
        main_thread = threading.current_thread()
        time_limit = 5 * 60  # 5분 (초 단위)
        time.sleep(time_limit)
        loop_thread.join()  # 백그라운드 스레드가 끝날 때까지 대기

        # 루프가 종료되었음을 알리는 메시지 출력
        print("루프가 종료되었습니다.")

    def check_status(self, client_name, push_point, current_point):
        while True:
            # 무한 루프에서는 특정 조건이 충족될 때까지 계속해서 데이터베이스를 확인합니다.
            charge_status = PointList.objects.filter(client_name=client_name,
            ...
            # 특정 조건이 충족되지 않았을 때는 일정 시간 동안 대기한 후 다시 확인합니다.
